  ...
  if (!targetId || !adj) {
    return (
      <div className="px-3 py-3 text-[11px] leading-relaxed text-ps-muted">
        选中一个<b className="text-ps-dim">图片图层</b>后可在此实时调整亮度、对比度、饱和度等参数。
      </div>
    )
  }
  ...
  return (
    <div>
      <div className="px-3 pb-1.5">
        <div className="mb-1.5 text-[11px] text-ps-dim">滤镜预设</div>
      ...
      <Slider label="曝光" value={adj.exposure} min={-100} max={100} resetTo={0} onChange={(v) => patch({ exposure: v }, false)} onCommit={() => engine?.pushHistoryState()} />
      <Slider label="亮度" value={adj.brightness} min={-100} max={100} resetTo={0} onChange={(v) => patch({ brightness: v }, false)} onCommit={() => engine?.pushHistoryState()} />
      <Slider label="对比度" value={adj.contrast} min={-100} max={100} resetTo={0} onChange={(v) => patch({ contrast: v }, false)} onCommit={() => engine?.pushHistoryState()} />
      <Slider label="饱和度" value={adj.saturation} min={-100} max={100} resetTo={0} onChange={(v) => patch({ saturation: v }, false)} onCommit={() => engine?.pushHistoryState()} />
      <Slider label="色温" value={adj.temperature} min={-100} max={100} resetTo={0} onChange={(v) => patch({ temperature: v }, false)} onCommit={() => engine?.pushHistoryState()} suffix="" />
      <Slider label="模糊" value={adj.blur} min={0} max={100} resetTo={0} onChange={(v) => patch({ blur: v }, false)} onCommit={() => engine?.pushHistoryState()} />

      <div className="flex gap-1.5 px-3 py-2">
        <Toggle on={adj.grayscale} label="灰度" onClick={() => patch({ grayscale: !adj.grayscale })} />
        <Toggle on={adj.blackwhite} label="黑白" onClick={() => patch({ blackwhite: !adj.blackwhite })} />
        <Toggle on={adj.vintage} label="复古" onClick={() => patch({ vintage: !adj.vintage })} />
      </div>
